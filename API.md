<!-- Generated by documentation.js. Update this documentation by updating the source code. -->

### Table of Contents

- [ok][1]
  - [Parameters][2]
  - [Examples][3]
- [error][4]
  - [Parameters][5]
  - [Examples][6]
- [errors][7]
  - [Parameters][8]
  - [Examples][9]
- [test][10]
  - [Parameters][11]
  - [Examples][12]
- [transform][13]
  - [Parameters][14]
  - [Examples][15]
- [pipe][16]
  - [Parameters][17]
  - [Examples][18]
- [oneOf][19]
  - [Parameters][20]
  - [Examples][21]
- [required][22]
  - [Parameters][23]
  - [Examples][24]
- [optional][25]
  - [Parameters][26]
  - [Examples][27]
- [any][28]
  - [Examples][29]
- [boolean][30]
  - [Parameters][31]
  - [Examples][32]
- [number][33]
  - [Parameters][34]
  - [Examples][35]
- [string][36]
  - [Parameters][37]
  - [Examples][38]
- [date][39]
  - [Parameters][40]
  - [Examples][41]
- [object][42]
  - [Parameters][43]
  - [Examples][44]
- [array][45]
  - [Parameters][46]
  - [Examples][47]
- [toBoolean][48]
  - [Examples][49]
- [toNumber][50]
  - [Examples][51]
- [parseInt][52]
  - [Parameters][53]
  - [Examples][54]
- [parseFloat][55]
  - [Parameters][56]
  - [Examples][57]
- [toString][58]
  - [Examples][59]
- [toDate][60]
  - [Examples][61]
- [floor][62]
  - [Examples][63]
- [ceil][64]
  - [Examples][65]
- [round][66]
  - [Examples][67]
- [truncate][68]
  - [Examples][69]
- [integer][70]
  - [Parameters][71]
  - [Examples][72]
- [finite][73]
  - [Parameters][74]
  - [Examples][75]
- [trim][76]
  - [Examples][77]
- [toLower][78]
  - [Examples][79]
- [toUpper][80]
  - [Examples][81]
- [pattern][82]
  - [Parameters][83]
  - [Examples][84]
- [shape][85]
  - [Parameters][86]
  - [Examples][87]

## ok

Creates a valid result.

### Parameters

- `value` **[any][88]** a valid value

### Examples

```javascript
C.ok(42); // => { isOk: true, value: 42 }
```

Returns **[Object][89]** a valid result wrapping the value.

## error

Creates an invalid result with one error.

### Parameters

- `value` **[any][88]** an invalid value
- `message` **[any][88]** an error message
- `path` **[Array][90]** a sequence of keys leading to the invalid value
  (optional, default `[]`)

### Examples

```javascript
C.error(43, "is not the answer"); // => { isOk: false, errors: [ ... ] }
```

Returns **[Object][89]** an invalid result wrapping the error.

## errors

Creates an invalid result with multiple errors.

### Parameters

- `errors` **[Array][90]** a list of error objects

### Examples

```javascript
const result1 = C.error(41, "is lesser than the answer");
const result2 = C.error(43, "is greater than the answer");

C.errors([...result1.errors, ...result2.errors]); // => { isOk: false, errors: [ ... ] }
```

Returns **[Object][89]** an invalid result wrapping the errors.

## test

Creates a check function that fails if a given predicate fails.

All arguments passed to the check function are passed to the predicate function.

### Parameters

- `predicate` **[Function][91]** a function returning a boolean
- `message` **[any][88]** an error message (optional, default `"is invalid"`)

### Examples

```javascript
const check = C.test(value => value === 42);

check(42); // => { isOk: true, ... }
check(43); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## transform

Creates a check function that transforms a value into another.

All arguments passed to the check function are passed to the transform function.

### Parameters

- `transform` **[Function][91]** a function transforming a value into another

### Examples

```javascript
const check = C.transform(value => value / 2);

check(42); // => { isOk: true, value 21 }
```

Returns **[Function][91]** a check function.

## pipe

Creates a check function that chains a set of child check functions.

The chain stops at the first failure.

All additional arguments passed after the value to the parent check function are
passed to the child check functions.

### Parameters

- `checks` **...[Function][91]** check functions to chain

### Examples

```javascript
const check = C.pipe(
  C.integer(),
  C.transform(value => value * 2),
  C.max(99),
);

check(25); // => { isOk: true, value: 50 }
check(50); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## oneOf

Creates a check function that returns the result from the first given check
function that succeeds.

If all child check functions fail, the last failing result is returned.

All additional arguments passed after the value to the parent check function are
passed to the child check functions.

### Parameters

- `checks` **...[Function][91]** check functions

### Examples

```javascript
const check = C.oneOf(C.integer(), C.string());

check(42); // => { isOk: true, ... }
check("jerome"); // => { isOk: true, ... }
check(true); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## required

Creates a check function that fails if a value is null or undefined.

### Parameters

- `message` **[any][88]** an error message (optional, default `"is required"`)

### Examples

```javascript
const check = C.required();

check(42); // => { isOk: true, ... }
check(null); // => { isOk: false, ... }
check(undefined); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## optional

Creates a check function that succeeds if a value is null or undefined or if a
given check function succeeds.

### Parameters

- `check` **[Function][91]** a check function

### Examples

```javascript
const check = C.optional(C.number());

check(null); // => { isOk: true, value: null }
check(undefined); // => { isOk: true, value: null }
check(42); // => { isOk: true, value: 42 }
check("jerome"); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## any

Creates a check function that always succeeds.

### Examples

```javascript
const check = C.any();

check(42); // => { isOk: true, value: 42 }
check("forty-two"); // => { isOk: true, value: "forty-two" }
```

Returns **[Function][91]** a check function.

## boolean

Creates a check function that fails when the value is not a boolean.

### Parameters

- `message` **[any][88]** an error message (optional, default
  `"is not a boolean"`)

### Examples

```javascript
const check = C.boolean();

check(true); // => { isOk: true, ... }
check(42); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## number

Creates a check function that fails when the value is not a number.

### Parameters

- `message` **[any][88]** an error message (optional, default
  `"is not a number"`)

### Examples

```javascript
const check = C.number();

check(42); // => { isOk: true, ... }
check("42"); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## string

Creates a check function that fails when the value is not a string.

### Parameters

- `message` **[any][88]** an error message (optional, default
  `"is not a string"`)

### Examples

```javascript
const check = C.string();

check("forty-two"); // => { isOk: true, ... }
check(42); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## date

Creates a check function that fails when the value is not a date.

### Parameters

- `message` **[any][88]** an error message (optional, default `"is not a date"`)

### Examples

```javascript
const check = C.date();

check(new Date()); // => { isOk: true, ... }
check(42); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## object

Creates a check function that fails when the value is not an object.

### Parameters

- `message` **[any][88]** an error message (optional, default
  `"is not an object"`)

### Examples

```javascript
const check = C.object();

check({}); // => { isOk: true, ... }
check(42); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## array

Creates a check function that fails when the value is not an array.

### Parameters

- `message` **[any][88]** an error message (optional, default
  `"is not an array"`)

### Examples

```javascript
const check = C.array();

check([]); // => { isOk: true, ... }
check({}); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## toBoolean

Creates a check function that converts a value of any type into a boolean.

### Examples

```javascript
const check = C.toBoolean();

check(1); // => { isOk: true, value: true }
check(0); // => { isOk: true, value: false }
```

Returns **[Function][91]** a check function.

## toNumber

Creates a check function that converts a value of any type into a number.

### Examples

```javascript
const check = C.toNumber();

check("42"); // => { isOk: true, value: 42 }
check(true); // => { isOk: true, value: 1 }
```

Returns **[Function][91]** a check function.

## parseInt

Creates a check function that tries to parse a string into an integer.

### Parameters

- `radix` **[number][92]** an integer representing the base used to parse the
  number
- `message` **[any][88]** an error message used when the string could not be
  parsed

### Examples

```javascript
const check = C.parseInt();

check("123"); // => { isOk: true, value: 123 }
check("abc"); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## parseFloat

Creates a check function that tries to parse a string into a floating point
number.

### Parameters

- `message` **[any][88]** an error message used when the string could not be
  parsed

### Examples

```javascript
const check = C.parseFloat();

check("3.14"); // => { isOk: true, value: 3.14 }
check("abc"); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## toString

Creates a check function that converts a value of any type into a string.

### Examples

```javascript
const check = C.toString();

check(42); // => { isOk: true, value: "42" }
check(true); // => { isOk: true, value: "true" }
check(null); // => { isOk: true, value: "" }
```

Returns **[Function][91]** a check function.

## toDate

Creates a check function that converts a value of any type into a date.

### Examples

```javascript
const check = C.toDate();

check("2020-09-03"); // => { isOk: true, value: Date(...) }
check(1583749507431); // => { isOk: true, value: Date(...) }
```

Returns **[Function][91]** a check function.

## floor

Creates a check function that rounds a number down to the previous integer.

### Examples

```javascript
const check = C.floor();

check(3.14); // => { isOk: true, value: 3 }
check(3); // => { isOk: true, value: 3 }
```

Returns **[Function][91]** a check function.

## ceil

Creates a check function that rounds a number up to the next integer.

### Examples

```javascript
const check = C.ceil();

check(3.14); // => { isOk: true, value: 4 }
check(3); // => { isOk: true, value: 3 }
```

Returns **[Function][91]** a check function.

## round

Creates a check function that rounds a number to the nearest integer.

### Examples

```javascript
const check = C.round();

check(3.14); // => { isOk: true, value: 3 }
check(3.86); // => { isOk: true, value: 4 }
```

Returns **[Function][91]** a check function.

## truncate

Creates a check function that transforms a number into its integer part.

### Examples

```javascript
const check = C.truncate();

check(3.14); // => { isOk: true, value: 3 }
check(-3.14); // => { isOk: true, value: -3 }
```

Returns **[Function][91]** a check function.

## integer

Creates a check function that fails when the value is not an integer.

### Parameters

- `message` **[any][88]** an error message (optional, default
  `"is not an integer"`)

### Examples

```javascript
const check = C.integer();

check(42); // => { isOk: true, ... }
check(3.14); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## finite

Creates a check function that fails when the value is not a finite number.

### Parameters

- `message` **[any][88]** an error message (optional, default
  `"is not a finite number"`)

### Examples

```javascript
const check = C.finite();

check(42); // => { isOk: true, ... }
check(Infinity); // => { isOk: false, ... }
check(NaN); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## trim

Creates a check function that trims a string.

### Examples

```javascript
const check = C.trim();

check("  jerome   "); // => { isOk: true, value: "jerome" }
```

Returns **[Function][91]** a check function.

## toLower

Creates a check function that converts a string to lower case.

### Examples

```javascript
const check = C.toLower();

check("JEROME"); // => { isOk: true, value: "jerome" }
```

Returns **[Function][91]** a check function.

## toUpper

Creates a check function that converts a string to upper case.

### Examples

```javascript
const check = C.toUpper();

check("jerome"); // => { isOk: true, value: "JEROME" }
```

Returns **[Function][91]** a check function.

## pattern

Creates a check function that fails if a string doesn't match a regular
expression.

### Parameters

- `regexp` **[RegExp][93]** a regular expression to test against
- `message` **[any][88]** an error message (optional, default `"is invalid"`)

### Examples

```javascript
const check = C.pattern(/[a-z]/);

check("jerome"); // => { isOk: true, ... }
check("42"); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

## shape

Creates a check function that will run different check functions on given
properties of an object.

### Parameters

- `checks` **[Object][89]** an object containing check functions

### Examples

```javascript
const check = C.shape({
  name: C.string(),
  age: C.number(),
});

check({
  name: "jerome",
  age: 42,
}); // => { isOk: true, ... }
check({
  name: "jerome",
  age: "forty-two",
}); // => { isOk: false, ... }
```

Returns **[Function][91]** a check function.

[1]: #ok
[2]: #parameters
[3]: #examples
[4]: #error
[5]: #parameters-1
[6]: #examples-1
[7]: #errors
[8]: #parameters-2
[9]: #examples-2
[10]: #test
[11]: #parameters-3
[12]: #examples-3
[13]: #transform
[14]: #parameters-4
[15]: #examples-4
[16]: #pipe
[17]: #parameters-5
[18]: #examples-5
[19]: #oneof
[20]: #parameters-6
[21]: #examples-6
[22]: #required
[23]: #parameters-7
[24]: #examples-7
[25]: #optional
[26]: #parameters-8
[27]: #examples-8
[28]: #any
[29]: #examples-9
[30]: #boolean
[31]: #parameters-9
[32]: #examples-10
[33]: #number
[34]: #parameters-10
[35]: #examples-11
[36]: #string
[37]: #parameters-11
[38]: #examples-12
[39]: #date
[40]: #parameters-12
[41]: #examples-13
[42]: #object
[43]: #parameters-13
[44]: #examples-14
[45]: #array
[46]: #parameters-14
[47]: #examples-15
[48]: #toboolean
[49]: #examples-16
[50]: #tonumber
[51]: #examples-17
[52]: #parseint
[53]: #parameters-15
[54]: #examples-18
[55]: #parsefloat
[56]: #parameters-16
[57]: #examples-19
[58]: #tostring
[59]: #examples-20
[60]: #todate
[61]: #examples-21
[62]: #floor
[63]: #examples-22
[64]: #ceil
[65]: #examples-23
[66]: #round
[67]: #examples-24
[68]: #truncate
[69]: #examples-25
[70]: #integer
[71]: #parameters-17
[72]: #examples-26
[73]: #finite
[74]: #parameters-18
[75]: #examples-27
[76]: #trim
[77]: #examples-28
[78]: #tolower
[79]: #examples-29
[80]: #toupper
[81]: #examples-30
[82]: #pattern
[83]: #parameters-19
[84]: #examples-31
[85]: #shape
[86]: #parameters-20
[87]: #examples-32
[88]: #any
[89]:
  https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object
[90]:
  https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Array
[91]:
  https://developer.mozilla.org/docs/Web/JavaScript/Reference/Statements/function
[92]: #number
[93]:
  https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/RegExp
